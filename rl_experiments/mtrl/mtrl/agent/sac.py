...
from mtrl.agent.mtl_optimizer import IMTLAgent, MGDAAgent
...
class Agent(AbstractAgent):
    ...
    def __init__(
        ...
        self._outer_agent = None
        self._naive_compute_grad = self._compute_gradient
        ...
        self._include_entropy_to_critic_loss = self.multitask_cfg.get('include_entropy_to_critic_loss', True)

        self._actor_use_naive_opt = self.multitask_cfg.get('actor_use_naive_opt', False)
        self._critic_use_naive_opt = self.multitask_cfg.get('critic_use_naive_opt', False)
        self._alpha_use_naive_opt = self.multitask_cfg.get('alpha_use_naive_opt', False)
    ...
    def _get_target_V(
        ...
        res = torch.min(target_Q1, target_Q2)
        if self._include_entropy_to_critic_loss:
            res = res - self.get_alpha(env_index=batch.task_obs).detach() * log_pi
        return res
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self._clip_targets:
                clipvals = batch.max_rewards / batch.reward_stds / (1 - self.discount)
                target_Q = torch.min(target_Q, clipvals)
        ...
        if self._critic_use_naive_opt:
            compute_grad = self._naive_compute_grad
            kwargs_to_compute_gradient = {"retain_graph": kwargs_to_compute_gradient["retain_graph"]}
            critic_loss = critic_loss.mean()
        else:
            compute_grad = self._compute_gradient
        ...
        if isinstance(self._outer_agent,IMTLAgent) or isinstance(self._outer_agent, MGDAAgent):
            al = self._outer_agent._mtl_optimizers['critic']._alpha_to_log<0
            logger.log("train/critic_neg_alphas", sum(al)/len(al), step)
            for i in range(len(al)):
                logger.log(f"train/critic_alphas_{i}", self._outer_agent._mtl_optimizers['critic']._alpha_to_log[i], step)
                #logger.log(f"train/critic_sts_{i}", self._outer_agent._mtl_optimizers['critic'].st[i], step)
    ...
    def update_actor_and_alpha(
        ...
        if self._actor_use_naive_opt:
            compute_grad = self._naive_compute_grad
            actor_kwargs_to_compute_gradient = {"retain_graph": kwargs_to_compute_gradient["retain_graph"]}
            actor_loss = actor_loss.mean()
        else:
            compute_grad = self._compute_gradient
            actor_kwargs_to_compute_gradient = kwargs_to_compute_gradient
        ...
        compute_grad(
            ...
            step=step,
            component_names=component_names,
            **actor_kwargs_to_compute_gradient,
        )

        self.actor_optimizer.step()
        if isinstance(self._outer_agent,IMTLAgent) or isinstance(self._outer_agent, MGDAAgent):
            al = self._outer_agent._mtl_optimizers['actor']._alpha_to_log<0
            logger.log("train/actor_neg_alphas", sum(al)/len(al), step)
            for i in range(len(al)):
                logger.log(f"train/actor_alphas_{i}", self._outer_agent._mtl_optimizers['actor']._alpha_to_log[i], step)
                #logger.log(f"train/actor_sts_{i}", self._outer_agent._mtl_optimizers['actor'].st[i], step)
        ...
        if self._alpha_use_naive_opt:
            compute_grad = self._naive_compute_grad
            alpha_kwargs_to_compute_gradient = {"retain_graph": kwargs_to_compute_gradient["retain_graph"]}
            alpha_loss = alpha_loss.mean()
        else:
            compute_grad = self._compute_gradient
            alpha_kwargs_to_compute_gradient = kwargs_to_compute_gradient
        ...
        compute_grad(
            ...
            **alpha_kwargs_to_compute_gradient,
        )
    ...
    def update(
        ...
        Args:
            ...
            return_maxes: array with maximum rollout returns to do target normalisation
        ...
        self.update_critic(
            ...
            return_maxes=torch.tensor(return_maxes, device=self.device, dtype=torch.float32)[batch.task_obs],
        )
    ...
    def add_outer_agent_ref(self, agent):
        # needed for logging IMTL/MGDA coefficients
        self._outer_agent = agent
