...
class Agent(abc.ABC):
    def __init__(
        ...
        self._clip_targets = multitask_cfg.get('clip_targets', False)
...
def _load_component_or_optimizer(
    ...
    if os.path.exists(path_to_load_from):
        if name == "log_alpha":
            component_or_optimizer = torch.load(path_to_load_from, map_location=target_device)
        else:
            component_or_optimizer.load_state_dict(torch.load(path_to_load_from, map_location=target_device))
    else:
