...
class Actor(BaseActor):
    def __init__(
        ...
        if "actor_cfg" in multitask_cfg:
            self.should_condition_model_on_task_id = multitask_cfg.actor_cfg.get("should_condition_model_on_task_id", False)
    ...
    def make_model(
        ...
        if self.should_condition_model_on_task_id:
            model_input_dim += task_embedding_dim
    ...
    def forward(
        ...
        if self.should_condition_model_on_task_id:
            obs = torch.cat((obs, self.task_embedder(mtobs.task_info.env_index).squeeze()), dim=1)
