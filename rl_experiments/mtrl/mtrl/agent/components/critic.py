...
class QFunction(base_component.Component):
    def __init__(
        ...
        self.should_condition_model_on_task_id = False
        ...
        if "critic_cfg" in multitask_cfg and multitask_cfg.critic_cfg:
            ...
            self.should_condition_model_on_task_id = multitask_cfg.critic_cfg.get("should_condition_model_on_task_id", False)

        self.should_use_multi_head_policy = check_if_should_use_multi_head_policy(
            multitask_cfg=multitask_cfg
        )

        if self.should_condition_model_on_task_id:
            task_embedding_dim = multitask_cfg['num_envs']
            if self.should_condition_model_on_task_id:
                obs_dim += task_embedding_dim
                self.task_embedder = nn.Embedding(multitask_cfg['num_envs'], task_embedding_dim)
    ...
    def forward(self, mtobs: MTObs) -> TensorType:
        ...
        if self.should_condition_model_on_task_id:
            obs_action = torch.cat((obs_action, self.task_embedder(mtobs.task_info.env_index).squeeze()), dim=1)
