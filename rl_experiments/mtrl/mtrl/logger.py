...
class MetersGroup(object):
    def __init__(self, file_name, formating, mode: str, retain_logs: bool, supress_wandb: bool):
        ...
        self._supress_wandb = supress_wandb
    ...
    def _dump_to_wandb(self, data, step, prefix):
        wandb.log({f"{prefix}_{k}":v for k,v in data.items()}, step)

    def dump(self, step, prefix):
        if len(self._meters) == 0:
            return
        data = self._prime_meters()
        if not self._supress_wandb:
...
class Logger(object):
    def __init__(self, log_dir, config, retain_logs: bool = False):
        ...
        self._supress_logger_wandb = False
        if config.get('label', '') == 'eval':
            self._supress_logger_wandb = True

        if not self._supress_logger_wandb:
            wandb.init(project="rct", config=self.config, group=config['label'])
        ...
        self.mgs = {
            key: MetersGroup(
                ...
                supress_wandb=self._supress_logger_wandb,
            )
