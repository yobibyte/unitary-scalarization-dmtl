        ...
        for step in range(self.start_step, exp_config.num_train_steps):
            ...
            if step % self.max_episode_steps == 0:  # todo
                if step > 0:
                    ...
                    for index, env_index in enumerate(env_indices):
                        self.return_maxes[index] = max(self.return_maxes[index], episode_reward[index])
                        self.logger.log(
            ...
            if step >= exp_config.init_steps:
                ...
                for _ in range(num_updates):
                    self.agent.update(self.replay_buffer, self.logger, step, return_maxes=self.return_maxes)
