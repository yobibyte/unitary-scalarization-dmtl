...
class Experiment(checkpointable.Checkpointable):
    def __init__(self, config: ConfigType, experiment_id: str = "0"):
        ...
        self.replay_buffer = hydra.utils.instantiate(
            ...
            task_obs_shape=(1,),
            action_shape=action_shape,
            n_tasks=self.config.env.num_envs,
            reward_clip_coeff=self.config.setup.get('reward_clip_coeff', -1),
            reward_normalisation=self.config.agent.multitask.get('buffer_reward_normalisation', False)
        )
