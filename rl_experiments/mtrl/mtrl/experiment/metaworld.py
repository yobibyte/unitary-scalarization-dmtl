...
class Experiment(multitask.Experiment):
    ...
    def __init__(self, config: ConfigType, experiment_id: str = "0"):
        ...
        self.return_maxes = np.zeros(self.config.env.num_envs, dtype=np.float32) 
    ...
    def evaluate_vec_env_of_tasks(self, vec_env: VecEnv, step: int, episode: int):
        ...
        if hasattr(self.agent, 'agent'):
            log_state_dict = self.logger.mgs['train']._meters
            opts = {'actor':self.agent.agent.actor_optimizer,
                    'critic':self.agent.agent.critic_optimizer,
                    'alpha': self.agent.agent.log_alpha_optimizer}
            for opt_name, opt in opts.items():
                # log scales for IMTL
                if hasattr(opt, 'st'):
                    for i, el in enumerate(opt.st):
                        log_state_dict[f'{opt_name}_opt_st_{i}'] = el
