...
class ReplayBufferSample:
    ...
    reward_stds: TensorType
    max_rewards: TensorType
...
class ReplayBuffer(object):
    ...
    def __init__(
        self, env_obs_shape, task_obs_shape, action_shape, capacity, batch_size, device, n_tasks=1, reward_clip_coeff=-1, reward_normalisation=False,
    ):
        ...
        self.counters = np.zeros(n_tasks) # counts number of frames for each of the environment
        self.first_moments = np.zeros(n_tasks, dtype=np.float32)
        self.second_moments = np.ones(n_tasks, dtype=np.float32)
        self.reward_clip_coeff = reward_clip_coeff
        self.reward_normalisation = reward_normalisation
        self.max_reward = np.ones(n_tasks, dtype=np.float32)*(-10000)
    ...
    def add(self, env_obs, action, reward, next_env_obs, done, task_obs):
        ...
        # keep track of reward moments to do normalisation when sampling
        # doing that at the environment level is bad because
        # similar states will have different normalised rewards based on the policy/timestep
        task_idx = task_obs.item()
        ...
        self.max_reward[task_idx] = max(self.max_reward[task_idx], reward)
    ...
    def get_reward_normalizer(self, env_indices):
        ...
        return torch.as_tensor(np.sqrt(var + 1e-8), device=self.device)
    ...
    def sample(self, index=None) -> ReplayBufferSample:
        ...
        sigma = self.get_reward_normalizer(env_indices)
        rewards = torch.as_tensor(self.rewards[idxs], device=self.device)
        if self.reward_normalisation:
            rewards /= sigma

        np_env_indices = env_indices.cpu().numpy()
        if self.reward_clip_coeff > 0:
            means = torch.tensor(self.first_moments[np_env_indices], device=self.device)
            if self.reward_normalisation:
                means = means/sigma
            rewards = torch.min(torch.max(rewards, means-self.reward_clip_coeff), means+self.reward_clip_coeff)

        rew_maxes = torch.as_tensor(self.max_reward[np_env_indices], device=self.device)

        return ReplayBufferSample(
            env_obses, actions, rewards, next_env_obses, not_dones, env_indices, idxs, sigma, rew_maxes
        )
    ...
    def sample_an_index(
        ...
        if self.reward_clip_coeff > 0:
            ...
            if self.reward_normalisation:
                means = means / sigma
            rewards = torch.min(torch.max(rewards, means - self.reward_clip_coeff),
                                means + self.reward_clip_coeff)
